...
import 'package:vst/services/datas.dart'; // Importation de la classe Datas pour récupérer les données
import 'package:youtube_player_flutter/youtube_player_flutter.dart'; // Importation du package Youtube Player Flutter

class VideoCard extends StatefulWidget {
  const VideoCard({super.key, required this.videoId, required this.index});
  final int index; // Index de la vidéo
  final String videoId; // ID de la vidéo

  @override
  State<VideoCard> createState() => _TutoCardState();
}

class _TutoCardState extends State<VideoCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<Datas>(
      builder: (context, datas, child) {
        final isNotEmpty = datas
            .videos
            .isNotEmpty; // Vérifie si la liste des tutoriels n'est pas vide

        final videos =
            datas.videos; // Liste des tutoriels provenant de l'API Youtube
        final videosViews = datas.videosViews; // Liste de tutoriels prédéfinis
        ...
        final title = isNotEmpty
            ...
            : videosViews[widget.index].title;
        final date = isNotEmpty
            ? videos[widget.index].publishedAt
            : videosViews[widget.index].publishedAt;
        final channelName = isNotEmpty
            ? videos[widget.index].channelName
            : videosViews[widget.index].channelName;
